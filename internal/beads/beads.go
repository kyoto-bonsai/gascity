...
type Bead struct {
	...
	//
	// Distinct from the older IndefinitelyDeferred bool above: that field is
	// wired specifically through normalizedBdReadState for the event-wire
	// reconstruction path and does not (as of this merge) feed
	// IsStatusDispatchable; this field is IsStatusDispatchable's own signal,
	// tri-state so a backend that cannot answer at all (nil) is distinguishable
	// from one that confirms "not indefinitely deferred" (false).
