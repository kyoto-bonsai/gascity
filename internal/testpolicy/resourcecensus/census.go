...
var bootstrapPolicy = Ledger{
	...
	AuditBaseline: []Baseline{
		{
			...
			ReportedCalls:   495,
			ReportedFiles:   135,
			OwnerBead:       "ga-f27exc.2.1",
			Invariant:       "tracked test source totals remain visible as audit evidence",
			ResourceOwner:   "ga-f27exc.2.1 owns the two new tagged process boundary proofs and the 2026-10-01 review of this expanded audit",
	...
	Debt: []Baseline{
		...
		},
		{
			...
			ResourceOwner:   "ga-f27exc.2.1 owns the added fail-if-contacted loopback proof; each owning test closes its server or removes duplicate coverage",
	...
	SmallDebt: []Baseline{
		...
		},
		{
			...
			ResourceOwner:   "ga-f27exc.2.1 owns the added fail-if-contacted loopback proof; non-Medium owners replace or classify retained listeners",
			MigrationTarget: "P0.4c",
			Expires:         "2026-10-01",
		},
		{
