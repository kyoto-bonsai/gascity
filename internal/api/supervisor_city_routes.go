...
func (sm *SupervisorMux) registerCityRoutes() {
	...
	// genclient/dashboard get one type discriminated by status. The op.Errors
	// list below drives Huma's enumerated problem+json error responses (400/401/
	// 403/404/409/501 plus the auto 422/500); it also suppresses the catch-all
	// `default`, so the manual 200/202 success bodies coexist with the P12 error
	// contract without a detail-less fallback.
