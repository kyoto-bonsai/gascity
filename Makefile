...
## gen-command-census: regenerate the Cobra command census (cmd/gc/metrics_census_gen.go),
## its product-metrics decode catalog, and the public example schema enum. Routed through
## `make` (not a raw `go run`) so it inherits the icu4c CGO exports above (ga-79sevz).
gen-command-census:
	go run ./cmd/gen-command-census
