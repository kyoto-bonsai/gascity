...
mail_compact_quarantine_alert() {
  ...
  # undelivered alert has to be as loud as the quarantine itself. A swallowed
  # return code is how a stale recipient alias went unnoticed for 68 days: the
  # caller just leaves quarantine_alert_emitted=0 and retries the same failing
  # send forever. Make the failure independently observable (events tail, gc
  # doctor) instead of only inferable from a never-incrementing notify_count,
  # in addition to the quarantine_notify_error detail record_marker_notify_state
  # persists into the marker file for later diagnosis.
  ...
  printf 'compact: db=%s CRITICAL: quarantine alert mail to %s FAILED — quarantine alert did not reach recipient (unreachable/invalid?), integrity issue may be going unnoticed: %s\n' \
...
flatten_database() {
  ...
  # verify_counts' verdict depends on. Deliberately does NOT also set on HEAD
  # movement (writer_race_detected's own signal): that case is already owned,
  # more rigorously, by the same-count-UPDATE diff-proof check above (it reads
  # gain_drift_is_additive_only, not just a moved HEAD) — folding it in here
  # too would let this gate defer a drift whose diff-proof just failed there.
  table_pass_race_detected=0
