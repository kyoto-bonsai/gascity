...
case "$query" in
  ...
  *"DOLT_DIFF("*"'beads')"*)
    ...
    case "$mode" in
      writer_race_same_count_hash_drift_only|same_count_hash_drift_with_writer_race|continuous_writer_same_count_drift|continuous_writer_planted_corruption)
        # continuous_writer_*: rows that are NOT purely additive between the
        # fixed pre/post-flatten commits — used by the fixed-commit fallback,
        # not the live additive-only proof, but this fixture's 'beads' arm is
        # the one every DOLT_DIFF('...', '...', 'beads') query matches
        # regardless of caller, so it must answer for both.
        print_cell 0
        ;;
      writer_race_same_count_hash_drift_diff_fails)
        print_cell 1
  ...
  *"DOLT_DIFF("*)
    ...
    # additive-only preservation proof and the fixed-commit fallback. Fallback
    # only: every DOLT_DIFF(..., 'beads') query, which is what every mode in
    # this fixture actually issues, is already answered by the 'beads' arm
    # above (case is first-match-wins) — this exists for a table name none of
    # the specific arms recognize.
