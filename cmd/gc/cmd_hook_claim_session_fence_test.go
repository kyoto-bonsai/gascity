...
func TestWriteHookClaimDrainStaleSessionWithDrainAck(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := writeHookClaimDrain(hookClaimLabel, hookClaimReasonStaleSession, true, true, false, fakeAck, &stdout, &stderr)
...
func TestWriteHookClaimDrainDoesNotAckWhenNotRequested(t *testing.T) {
	...
	var stdout, stderr bytes.Buffer
	code := writeHookClaimDrain(hookClaimLabel, hookClaimReasonStaleSession, true, false, false, fakeAck, &stdout, &stderr)
	if code != 1 {
