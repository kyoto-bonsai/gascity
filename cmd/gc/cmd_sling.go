...
Examples:
  ...
  echo "fix login" | gc sling mayor --stdin # read bead text from stdin`,
		...
		RunE: func(_ *cobra.Command, args []string) error {
			...
			code := 0
			if jsonOutput {
				code = cmdSlingWithJSON(args, formula, effectiveNudge, force, title, vars, merge, noConvoy, owned, reassign, onFormula, noFormula, fromStdin, dryRun, scopeKind, scopeRef, true, stdout, stderr)
			} else {
				code = cmdSling(args, formula, effectiveNudge, force, title, vars, merge, noConvoy, owned, reassign, onFormula, noFormula, fromStdin, dryRun, scopeKind, scopeRef, stdout, stderr)
			}
