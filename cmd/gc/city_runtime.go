...
func (cr *CityRuntime) beadReconcileTick(ctx context.Context, result DesiredStateResult, sessionBeads *sessionBeadSnapshot, trace *sessionReconcilerTraceCycle, bootReconcile bool) {
	...
	if bootReconcile {
		...
		phaseStart = time.Now()
		if closedWisps := sweepClosedTriggerWispSessions(cr.cityPath, sessStore.Store, rigStores, sessionBeads, cr.cfg, cr.sp, result.snapshotQueryPartial(), cr.rec, cr.stderr); len(closedWisps) > 0 {
			var sessionQueryPartial bool
			sessionBeads, sessionQueryPartial = cr.loadSessionBeadSnapshotWithPartial()
			result.SessionQueryPartial = result.SessionQueryPartial || sessionQueryPartial
		}
...
func sweepClosedTriggerWispSessions(
	...
	for _, info := range sessionBeads.OpenInfos() {
		...
		if !closeSessionInfoIfUnassigned(cityPath, store, rigStores, cfg, info, "gc_retired_closed_trigger", time.Now().UTC(), stderr) {
