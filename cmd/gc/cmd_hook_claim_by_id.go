...
func doHookClaimByID(id, dir string, opts hookClaimOptions, ops hookClaimOps, stdout, stderr io.Writer) int {
	...
	if result, claimedBead, ok := hookClaimExistingAssignment(candidates, opts); ok {
		// minted=false: adoption returns work this session already owned (see
		// the identical call in claimHookWorkWithRunner, cmd_hook_claim.go).
		return writeHookClaimWorkResultForBead(result, claimedBead, opts, ops, dir, false, stdout, stderr)
	}
...
func writeHookClaimByIDRefused(store beads.Store, bead beads.Bead, opts hookClaimOptions, claimsErrored bool, stdout, stderr io.Writer) int {
	...
	case hookClaimAlreadyHeldCollision(current, opts.RouteTargets):
		reason = hookClaimReasonClaimConflict
	}
