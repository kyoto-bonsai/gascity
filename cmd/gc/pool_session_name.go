...
// that have no error channel. The expectedStatus must match the snapshot
...
// in_progress→open) the snapshot's release decision is no longer safe. Open
// status is required for the issue #2793 path — graph.v2 step beads stuck on
// a dead session's long-form assignee are status=open, not in_progress.
//
