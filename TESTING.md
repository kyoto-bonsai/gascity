...
added by `cmd/gc/sling_remote_test.go:TestCmdSlingRemote_RefusesUnsupportedModes`
fails if a forbidden mode contacts the remote endpoint. The owner must migrate
these proofs to bounded process and listener fixtures or remove redundant
coverage before expiry; any retained call requires a renewed, reviewed baseline.
The nine tagged environment occurrences in
`internal/runtime/tmux/socket_env_integration_test.go` remain in the
all-source inventory but are outside the enforced **untagged** environment
ceiling. Measured sleep reductions are banked in the fixed-sleep baselines.
...
| --- | --- | --- | --- | --- | --- | --- |
| Audit baseline | all tracked test source | fixed_sleep: 479 calls / 172 files (historical regex census: 447 / 157) | ga-cp3hwi | tracked test source totals remain visible as audit evidence; ga-cp3hwi owns this point-in-time source census | P0.4a | 2026-10-01 |
...
| Audit baseline | all tracked test source | subprocess: 664 calls / 192 files (historical regex census: 495 / 135) | ga-f27exc.2.1 | tracked test source totals remain visible as audit evidence; ga-f27exc.2.1 owns the two new tagged process boundary proofs and the 2026-10-01 review of this expanded audit | P0.4a | 2026-10-01 |
...
| Small debt ratchet | all untagged test source | http_test_server: 319 calls / 66 files (historical regex census: 300 / 66) | ga-f27exc.2.1 | untagged Small HTTP test server call/file totals cannot grow; reductions must lower this baseline; ga-f27exc.2.1 owns the added fail-if-contacted loopback proof; non-Medium owners replace or classify retained listeners | P0.4c | 2026-10-01 |
...
| Source debt ratchet | all untagged test source | http_test_server: 319 calls / 66 files (historical regex census: 255 / 56) | ga-f27exc.2.1 | untagged HTTP test server call/file totals cannot grow; reductions must lower this baseline; ga-f27exc.2.1 owns the added fail-if-contacted loopback proof; each owning test closes its server or removes duplicate coverage | P0.4c | 2026-10-01 |
